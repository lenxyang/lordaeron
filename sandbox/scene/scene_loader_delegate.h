...
class SimpleSceneLoaderDelegate : public SceneNodeLoader {
 public:
  SimpleSceneLoaderDelegate(azer::Effect* effect)
      : effect_(effect) {}
  ...
  bool LoadSceneNode(SceneNode* node, azer::ConfigNode* config,
    ...
    if (type == "mesh") {
      ...
      azer::MeshPtr mesh = LoadMesh(mesh_node, loadctx->filesystem);
  ...
  azer::MeshPtr LoadMesh(azer::ConfigNode* config, azer::FileSystem* fsystem) {
    std::string pathstr;
    if (!config->GetChildText("path", &pathstr)) {
      return azer::MeshPtr();
    }

    ModelLoader loader(fsystem);
